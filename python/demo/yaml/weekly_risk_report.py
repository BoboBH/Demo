...
print("start to adjust postionEvents");
...
print("adjust postionEvents successfully");
...
# find last valid fx rate[bobo huang]
def getLastValidER(assetCcy, baseCcy, prices):
    if assetCcy != baseCcy:
        if assetCcy+baseCcy+' CURNCY' in prices.columns: #e.g. EURUSD
            ...
            return er
        elif baseCcy+assetCcy+' CURNCY' in prices.columns: #e.g. USDEUR
        ...
        return 1.
#add ‘ER' field on position Event, it can call getLastValidER instead of calcuating ER again.[bobo huang]        
...
def postionsByDate(positions_orgin, startDate, endDate, prices, selectMask, baseCCY): # dataframe , timestamp, series
    ...
    print("position unique Key on startdate=:", startDate, "enddate=", endDate)
    print(positions['Key'].unique())
    for k in positions['Key'].unique():     
        singleData = dict()
        #find the first one,order by date asc
        ...
        print("Key = ", k, "; ticker = ",ticker);
        print("security unique:")
        print(pos["Security"].unique())
...
def runNAV(thisPostionEvents, thisHistoricalData, datelist):
    ...
    for i in range(1, len(datelist)):
        d = datelist[i]
        d0 = datelist[i-1]
        print("d=",d, "d0=",d0,"press any key to continue...")
        #tmp_lin = sys.stdin.readline()
        #print(tmp_lin)
...
print("calculate and show weeky report,input a line to continue...")
sys.stdin.readline()
weeklyTime = [w for w in tmp if w.weekday() == 4]
if tmp[-1] > weeklyTime[-1] :
    weeklyTime.append(tmp[-1])
weeklyReturn = pandas.DataFrame(columns=['Date','PnL'])
for i in range(1,len(weeklyTime)):
    if weeklyTime[i] <= navDF['Date'].min():
        continue
    weeklyReturn = weeklyReturn.append({'Date':weeklyTime[i].date(),'PnL':navDF[navDF['Date'] == weeklyTime[i]]['Cum PnL'].values[-1] - navDF[navDF['Date'] == weeklyTime[i-1]]['Cum PnL'].values[-1]}, ignore_index=True)
    #print(weeklyTime[i],navDF[navDF['Date'] == weeklyTime[i]]['Cum PnL'].values[-1] - navDF[navDF['Date'] == weeklyTime[i-1]]['Cum PnL'].values[-1])
weeklyReturn.set_index('Date').plot(kind='bar',title='Weekly PnL')
print(weeklyReturn);
print("start run SPX INDEX, proess any key to continue...")
sys.stdin.readlin();
benchmark = 'SPX INDEX'
fig, ax = plt.subplots()
#ax.plot(navDF['Date'], navDF['Acc Adj Market Value'])
ax.plot(navDF['Date'], navDF['Cum PnL'])
ax.xaxis.set_tick_params(rotation=45)
axt=ax.twinx()
tmpBM = historicalData[historicalData['DATE'].isin(navDF['Date'].tolist())][benchmark]
navDF['Benchmark'] = (tmpBM/tmpBM.iloc[0]).values
axt.plot(navDF['Date'], tmpBM/tmpBM.iloc[0], 'r--')
#beta is not stable, not expose
tmpBeta = navDF[['Date','Acc Adj Market Value(USD)','Benchmark']].set_index('Date').pct_change().dropna()
tmpBetaDF = pandas.DataFrame(columns=['Date','Rolling Beta'])
for i in range(1,len(weeklyTime)):
    if weeklyTime[i] <= navDF['Date'].min():
        continue
    tmpBeta2 = tmpBeta[(tmpBeta.index<=weeklyTime[i])&(tmpBeta.index>weeklyTime[i-1])]
    tmpBetaDF = tmpBetaDF.append({'Date':weeklyTime[i].date(),'Rolling Beta':tmpBeta2.cov().values[0][1]/tmpBeta2.cov().values[1][1]},ignore_index=True)
    

ddV = [0]
ddVDate = [numpy.nan]
value = navDF['Acc Adj Market Value(USD)'].values
for i in range(1,len(value)):
    maxPre = max(value[0:i+1])
    maxPreLoc = navDF[navDF['Acc Adj Market Value(USD)'] == maxPre]['Date'].iloc[-1]
    ddV.append(min(0.,value[i] / maxPre -1))
    ddVDate.append(maxPreLoc)
navDF['ddv'] = ddV
navDF['ddvDate'] = ddVDate
#navDF['ddv'].plot()
navDF['ddv_rank'] = navDF['ddv'].rank()
ddvStartDate = navDF[navDF['ddv_rank'] == navDF['ddv_rank'].min()].iloc[-1]['ddvDate']
ddvEndDate = navDF[navDF['ddv_rank'] == navDF['ddv_rank'].min()].iloc[-1]['Date']
if ddvStartDate != ddvEndDate:
    print('Draw Rectangle',ddvStartDate, ddvEndDate)
    trans = mtransforms.blended_transform_factory(ax.transData, ax.transAxes)
    theta = 0.9
    ax.axes.get_ylim()
    ax.fill_between(navDF['Date'].values, ax.axes.get_ylim()[0], ax.axes.get_ylim()[1], where= (navDF['Date'] >= ddvStartDate) &(navDF['Date'] <= ddvEndDate), facecolor='grey', alpha=0.5)