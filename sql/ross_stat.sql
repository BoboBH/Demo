...
--A0F765E7-B739-44EC-9EFA-30EFFC86B846
...
create table #tmp_statement_stat(followerid uniqueidentifier,cnt int,hkd_amount decimal(19,4),direction int,stat_type int)
...
select   b.new_clientinfo ,isnull(c.new_yff_relationship_managerid,'A0F765E7-B739-44EC-9EFA-30EFFC86B846') from dev02.reohk_mscrm.dbo.new_yffcustomer a
...
insert into #tmp_client_follow_mapping(clientid, followerid)
select   b.new_clientinfo ,isnull(c.new_yff_relationship_managerid,'A0F765E7-B739-44EC-9EFA-30EFFC86B846') as new_yff_relationship_managerid  from dev02.reohk_mscrm.dbo.new_yffuser a
...
where b.new_registertime >= '2019-03-19' and b.new_registertime < DATEADD(dd,1,'2019-03-19')
group by followerid
...
where a.createdon>='2019-03-19' and a.createdon < DATEADD(dd,1,'2019-03-19')
...
select a.followerid,sum(b.hkd_amount) as hkd_amount,count(1) cnt,b.direction,1 as stat_type from #tmp_client_follow_mapping a
inner join #tmp_client_statement b on a.clientid = b.clientid and b.direction =1--deposit
where txdate>='2019-03-19' and txdate < DATEADD(dd,1,'2019-03-19')
group by a.followerid,direction

insert into #tmp_statement_stat(followerid,hkd_amount,cnt,direction,stat_type)
select a.followerid,sum(b.hkd_amount) as hkd_amount,count(1) cnt,b.direction,2 from #tmp_client_follow_mapping a
inner join #tmp_client_statement b on a.clientid = b.clientid and b.direction =1--deposit
group by a.followerid,b.direction

insert into #tmp_statement_stat(followerid,hkd_amount,cnt,direction,stat_type)
select a.followerid,sum(b.hkd_amount) as hkd_amount,count(1) cnt,b.direction,1 from #tmp_client_follow_mapping a
...
select a.followerid,a.name,
ISNULL(reg_cur_stat.cnt,0) as register_cnt, ISNULL(reg_his_stat.cnt,0) as total_reg_cnt,
ISNULL(acct_cur_stat.cnt,0) as acct_cnt, ISNULL(acct_his_stat.cnt,0) as total_acct_cnt,
ISNULL(deposit_cur_stat.hkd_amount,0) as dep_hkd_amount,ISNULL(deposit_cur_stat.cnt,0) as dep_cnt,
ISNULL(deposit_his_stat.hkd_amount,0) as total_dep_hkd_amount,ISNULL(deposit_his_stat.cnt,0) as total_dep_cnt,
ISNULL(withdrawal_cur_stat.hkd_amount,0) as wid_hkd_amount,ISNULL(withdrawal_cur_stat.cnt,0) as wid_cnt,
ISNULL(withdrawal_his_stat.hkd_amount,0) as total_wid_hkd_amount,ISNULL(withdrawal_his_stat.cnt,0) as total_wid_cnt
 from #tmp_rm a
