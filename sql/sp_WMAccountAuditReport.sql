...
--Description:WMAccount Audit Report
--PS:Replace dev02.reohk_mscrm. with empty string while load on target server;
--Sample exec dbo.sp_WMAccountAuditReport '2019-02-18'
USE ReoHK_MSCRM
GO

IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[sp_WMAccountAuditReport]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1) 
	DROP PROCEDURE dbo.sp_WMAccountAuditReport
GO
CREATE PROCEDURE dbo.sp_WMAccountAuditReport(@p_EndDate DATETIME = NULL) AS
BEGIN
	...
	insert into #tmp_attr(entname,entdisplayname, colname, dispalyname)
		...
		WHERE   LocalizedLabelView_1.ObjectColumnName = 'LocalizedName'
		 ...
		 AND EntityView.Name IN ('new_yffwmaccount')
	...
	inner join dev02.reohk_mscrm.dbo.new_yffwmaccountBase b  with(nolock) on a.ObjectId = b.new_yffwmaccountId
	...
	INNER JOIN dev02.reohk_mscrm.dbo.systemuser u  with(nolock) on a.UserId = u.SystemUserId
	 ...
	 inner join dev02.reohk_mscrm.dbo.new_yffwmaccountBase c  with(nolock) on c.new_yffwmaccountId = b.ObjectId
	 ...
	 where e.LogicalName = 'new_yffwmaccount' and  d.LogicalName in
	 ('new_feecode')
	 ...
	 select new_yffwmaccountId as ObjectId,CAST(colname as varchar(max)) as colname, val from( 
	 select a.new_yffwmaccountId, 
	CAST(new_feecode                  AS NVARCHAR(MAX)) AS new_feecode              
	  from  dev02.reohk_mscrm.dbo.new_yffwmaccountBase a with(nolock) 
	 where new_yffwmaccountId in(select ObjectId from #tmp_change)
	 ) as src  
	UNPIVOT (
			val FOR colname IN 
			(new_feecode)) AS UNPVT;
	...
	select  a.ObjectId,b.new_wmclientid AS WM_ClientId, a.colName,
	...
	) bcu on fr.ObjectId = bcu.ObjectId and bcu.colname = fr.colname
...
END
GO
GRANT EXEC ON  dbo.sp_WMAccountAuditReport TO cdbdev
GO

--exec dbo.sp_WMAccountAuditReport '2019-08-29'