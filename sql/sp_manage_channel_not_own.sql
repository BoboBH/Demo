...
--sample:exec dbo.sp_manage_channel_not_own
USE ReoHK_MSCRM
GO

IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[sp_manage_channel_not_own]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1) 
	DROP PROCEDURE dbo.sp_manage_channel_not_own
GO
CREATE PROCEDURE dbo.sp_manage_channel_not_own(@p_EndDate DATETIME = NULL) AS
BEGIN
