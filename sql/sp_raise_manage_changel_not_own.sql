...
--sample:exec dbo.sp_raise_manage_changel_not_own
USE ReoHK_MSCRM
GO

IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[sp_raise_manage_changel_not_own]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1) 
	DROP PROCEDURE dbo.sp_raise_manage_changel_not_own
GO
CREATE PROCEDURE dbo.sp_raise_manage_changel_not_own(@p_EndDate DATETIME = NULL) AS
BEGIN
	...
	--Mangaes channel but not owns the channel
	...
	left join #tmp_managed_channel b on a.userid = b.userid and a.channel_code = b.channel_code and b.source =2
	where a.source = 1 and b.userid is null
END
GO