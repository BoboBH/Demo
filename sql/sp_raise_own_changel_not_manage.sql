...
--sample:exec dbo.sp_raise_own_changel_not_manage
USE ReoHK_MSCRM
GO

IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[sp_raise_own_changel_not_manage]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1) 
	DROP PROCEDURE dbo.sp_raise_own_changel_not_manage
GO
CREATE PROCEDURE dbo.sp_raise_own_changel_not_manage(@p_EndDate DATETIME = NULL) AS
BEGIN
